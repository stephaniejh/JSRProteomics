...
---
title: "BioIn_2025_12_v3.4.2_ClusterProfiler_EExAge_vis_testing_v1"
...
[*BioIn_2025_12_v3.4.2_ClusterProfiler_EExAge_vis_testing_v1*]{.underline}

### *2026_01_28 - Got too messy but didn't want to delete, made new v2 instead - [BioIn_2026_01_v3.4.2_ClusterProfiler_EExAge_vis_v2]{.underline}*

ADD NOTES LATER VERY MESSY!
...
```{r}
SynGO_ids <- read_excel("input/db/SynGO_idmap_entrez_2026_02_01.xlsx")
```

```{r}
protein_comp_entrez_syngo <- left_join(protein_comp_entrez, SynGO_ids,
                                       by = join_by(ENTREZID == query))

```

```{r}
# write_excel_csv(protein_comp_entrez_syngo,
#                 "output/2026_02_01_EExAge_protein_comp_entrez_syngo.csv")
```
...
```{r}
neuron_map_2026_01_28 <- drawCell::drawCell(organism_identifier = "6072",
                                            list_sl_colors = list(
...
-   To detect clusters in the PPI network, use get_clusters() method, which in turn uses the clustering algorithim in cluster\_ in igraph functions - see `??igraph::cluster_` 
...
```{r}
# 2026_02_01 - same as above but with SynGO mapped
protein_comp_entrez_syngo_top_250 <- protein_comp_entrez_syngo %>%
  ungroup() %>%
  group_by(Label) %>%
  slice_min(adj.pvalue, n = 250, with_ties = FALSE)

split_protein_comp_entrez_syngo_top_250 <- split(protein_comp_entrez_syngo_top_250,
                                           protein_comp_entrez_syngo_top_250$Label)

# # write out excel!
# write_xlsx(split_protein_comp_entrez_syngo_top_250,
#            "output/2026_02_01_AgeXEE_split_protein_comp_entrez_SynGO_top_250.xlsx")


protein_comp_entrez_syngo_top_250_ranked <- protein_comp_entrez_syngo_top_250 %>%
  mutate(top_250_ranking = sign(log2FC)*(-log10(adj.pvalue)))

protein_comp_entrez_syngo_top_250_ranked_split <-  split(
  protein_comp_entrez_syngo_top_250_ranked,
  protein_comp_entrez_syngo_top_250_ranked$Label)

# # write out excel!
# write_xlsx(protein_comp_entrez_syngo_top_250_ranked_split,
#            "output/2026_02_03_AgeXEE_split_protein_comp_entrez_SynGO_top_250_ranking.xlsx")



protein_comp_entrez_syngo_ranked <- protein_comp_entrez_syngo %>%
  ungroup() %>%
  mutate(ranking = sign(log2FC)*(-log10(pvalue))) %>%
  relocate(symbol, .before = ranking)


split_protein_comp_entrez_syngo_ranked <- split(
  protein_comp_entrez_syngo_ranked,
  protein_comp_entrez_syngo_ranked$Label)

#  # write out excel!
# write_xlsx(split_protein_comp_entrez_syngo_ranked,
#  "output/2026_02_03_AgeXEE_split_protein_comp_entrez_SynGO_full_entrez_ranking.xlsx")
...
```{r}
#USE FOR STRINGDB INPUT! | by uniprot protein accession
protein_ranked <- protein_comp %>%
  group_by(Label) %>%
  mutate(ranking = sign(log2FC)*(-log10(pvalue))) %>%
  group_by(Label, Protein) %>%
  summarise(ranking = mean(ranking, na.rm = TRUE)) %>%
  arrange(Label, -ranking)

# SPLIT
protein_ranked_split <- split(protein_ranked,
                              protein_ranked$Label)

#  # write out excel!
# writexl::write_xlsx(protein_ranked_split,
#            "output/2026_01_30_AgexEE_split_protein_comp_ranked_by_uniprot_acc.xlsx")
```

```{r}
#USE FOR STRINGDB INPUT! | ranked by uniprot primary gene name from FASTA
gene_ranked <- protein_comp %>%
  group_by(Label) %>%
  mutate(ranking = sign(log2FC)*(-log10(pvalue))) %>%
  group_by(Label, gene) %>%
  summarise(ranking = mean(ranking, na.rm = TRUE)) %>%
...
# SPLIT
gene_ranked_split <- split(gene_ranked,
                              gene_ranked$Label)

#  # write out excel!
# writexl::write_xlsx(gene_ranked_split,
#            "output/2026_01_30_AgexEE_split_protein_comp_ranked_by_gene_name.xlsx")
```
