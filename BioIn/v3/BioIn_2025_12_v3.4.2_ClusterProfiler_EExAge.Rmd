...
1.  2026_01_12 - GO redo & KEGG redoredo, Msigdb Hallmarks & C8 celltypes redo
    ...
    5.  [Msigdb C8 Celltypes_via_GSEA]{.underline} ➝ gsea_msig_C8_ct_EExAge_mss3.2.2_t1_cp3.4.2_t2 ➝ 2026_01_12_gsea_msig_C8_ct_EExAge_mssv3.2.2_t1_cpv3.4.2_t2.rda
...
#      file = "output/gsea_msig_C8_ct_EExAge_mss3.2.2_t1_cp3.4.2_t2gsea_msig_C8_ct_EExAge_mss3.2.2_t1_cp3.4.2_t2")
```
...
-   <https://www.gsea-msigdb.org/gsea/msigdb/mouse/geneset/RAMPON_ENRICHED_LEARNING_ENVIRONMENT_EARLY_DN.html>

```{r}
# NONE SIG!!! even with lower min size!  - ignore
msig_Rampon_EE <- msig_rat_df %>% 
  filter(gs_collection == "C2") %>%
  filter(str_detect(gs_name, "RAMPON")) %>%
  # filter(str_detect(gs_name, "NEURON")) %>%
  dplyr::select(gs_name, ncbi_gene)
```
...
# Container - Emty Vector
gsea_msig_c2_reactome_res_list <- vector(mode = "list")
```
...
for (lb in label_names) {
  ...
  gsea_msig_c2_reactome_res_list$geneList[[lb]] <- geneList_working_df
  ...
  # PART 2 - run gsea
  msig_res_working <- clusterProfiler::GSEA(geneList = geneList_vec_working,
                                              TERM2GENE = msig_c2_reactome,
                                              exponent = 1,
                                              minGSSize = 10,
                                              maxGSSize = 500,
                                              eps = 1e-10,
  ...
  #save gsea results 
  gsea_msig_c2_reactome_res_list$gsea_results[[lb]] <- msig_res_working
  gsea_msig_c2_reactome_res_list$gsea_results_tib[[lb]] <- tibble::as_tibble(msig_res_working)
  

  
  # PART 3 - if else for dot plot to deal with any nas
  if (nrow(msig_res_working@result) > 0 ) {
    
  # PART 3 - PLOTS 
  # sign dot plots 
  # 3.1 - dot plot split by sign
  dp_sign <- enrichplot::dotplot(msig_res_working,
                            split = '.sign',
                            showCategory = 30,
                            font.size = 11,
                            title = paste0(stringr::str_glue(
                              'Housing x Age - {lb} - msigdbr C2 reactome - 2026_02_09 - via cp_gsea dotplot by sign  - 1pspg MssTMT (v3.2.2)'))) +
    facet_grid(.~.sign)
  
  print(dp_sign) # print dot plots 
  gsea_msig_c2_reactome_res_list$dotplots_sign[[lb]] <- dp_sign # save to list
...
```{r}
# horrific name but at least clear...
gsea_msig_c2_reactome_EExAge_mss3.2.2_t1_cp3.4.2  <- gsea_msig_c2_reactome_res_list

# save(gsea_msig_c2_reactome_EExAge_mss3.2.2_t1_cp3.4.2,
#      file = "output/2026_02_09_gsea_msig_c2_reactome_EExAge_mss3.2.2_t1_cp3.4.2.rda")
```
