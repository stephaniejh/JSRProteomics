...
KEGG_tibs_df_6_anno <- left_join(KEGG_tibs_df_6, KEGG_cat_anno, 
  ...
  mutate(my_cat_color = case_when(
    category == "Metabolism" ~ "#f5d2d3",
    category == "Genetic Information Processing" ~ "#BED0E2",
    category == "Organismal Systems" ~ "#dfccf1",
    category == "Cellular Processes" ~ "#f7e1d3",
    category == "Environmental Information Processing" ~ "#bdd0c4",
    category == "Human Diseases" ~ "#e3e4e6"
  ))

my_cat_color_vec <- setNames(object = KEGG_tibs_df_6_anno$my_cat_color,
                             KEGG_tibs_df_6_anno$subcategory_2) # needs 2 be name vec
...
lolliplot <- function(data,
  ...
  lol <- ggplot(data,
    ...
    # facet_nested(facet_formula, # KEGG subcategory anno!
    #              space = "free", scales = "free") +
     
    # facet_grid2(facet_formula, # KEGG subcategory anno! half working ....
    #              space = "free", scales = "free",
    #             strip = strip_themed(
    #               background_y = elem_list_rect(fill = cat_color))) +
...
KEGG_lolli_cat <- KEGG_tibs_df_6_anno  %>%
  
  filter(!(Description %in% human_diseases)) %>%
  filter(!(Description %in% organs)) %>%
  
  ggplot(
    aes(x = reorder(Description_2, NES),
        y = NES)) +
  
  geom_segment(aes(x = Description_2,
                   yend = NES, y = 0,
                   alpha = p.adjust)) +
  geom_point(aes(alpha = p.adjust,
                 size = setSize,
                 fill = NES),
             shape = 21) +
  scale_alpha(range = c(.95,.3)) + # rev alpha so darker = more sig | all
  ...
  labs(subtitle = "KEGG GSEA",
       y = 'NES' ) +
  
  coord_flip() + #fliptheplot
  
  facet_grid2(reorder(subcategory_2, -NES)~Label, # KEGG subcategory anno! 
              space = "free", scales = "free",
              strip = strip_themed(
                background_y = elem_list_rect( # DOES NOT WORK AS INTENDED!??!?!?!
                  fill = KEGG_tibs_df_6_anno$my_cat_color)
                  )) +
...
for(kp in kegg_POI) {
  ...
  vol_plot <- protein_comp_entrez %>%
    ...
    # geom_text_repel(data = my_kp_prot, # LABELED GENES!!! | works but too messy
    #                 aes(x = log2FC, y = -log10(adj.pvalue),
    #                     label = gene,
...
```{r}
# plot_grid(plotlist = kegg_volcano_list[c(9:10,12,14, 11,13)], ncol = 2)
plot_grid(plotlist = kegg_volcano_list[1:14], ncol = 2)
...
# ggsave("2026_02_26_REACT_21_volcanos_300dpi.png", dpi = 300) # 11.4 x 5.99
```
