...
lolliplot <- function(data,
  ...
  lol <- ggplot(data,
    ...
    scale_alpha(range = c(.99,.39)) + #  GO TRIM VERSION so matches others
    ...
    scale_x_discrete(labels = scales::label_wrap(width = 30)) + # break text |
...
organs <- c("Pancreatic secretion", # digestive
            ...
            "Thermogenesis" # Envvironmental Adapation 
            )
```

### KEGG trimmed & subcategory

```{r}
# KEGG 6 TRIMMED W CAT ANNO
KEGG_lol_f6_trim <- KEGG_tibs_df_6_anno %>%
  # filter(!(Description %in% human_diseases)) %>%
  #   filter(!(Description %in% organs)) %>%
  lolliplot(                         
    facet_formula = reorder(subcategory_2, -NES)~Label,
    x_1 = Description_2,
    subtitle = "KEGG GSEA", legend_pos = "bottom")
...
### USED FOR MAIN THESIS

```{r}
# KEGG 3 EE TRIMMED W CAT ANNO
KEGG_tibs_EE_3_trim <- KEGG_tibs_df_6_anno %>%
  filter(Label %in% c("H_EE_vs_SH","HxA_EE_YNG_vs_SH_YNG","HxA_EE_OLD_vs_SH_OLD")) %>%
  filter(!(Description %in% human_diseases)) %>%
  filter(!(Description %in% organs)) %>%
  lolliplot(
    ...
    x_1 = Description_2,
    subtitle = "KEGG GSEA - Housing", legend_pos = "bottom")
...
```{r}
# KEGG 3 AGE TRIMMED
KEGG_tibs_AGE_3_trim <- KEGG_tibs_df_6_anno %>%
  ...
  filter(!(Description %in% organs)) %>%
  lolliplot( 
    # cat_color = my_cat_color,
    # cat_color = my_cat_color_vec, # ONLY NEEDED FOR KEGG CATEGORIES ????
    facet_formula = reorder(subcategory_2, -NES)~Label,
    x_1 = Description_2,
    subtitle = "KEGG GSEA - Age", legend_pos = "bottom")

KEGG_tibs_AGE_3_trim
```
...
```{r}
# plot_grid(KEGG_tibs_EE_3_trim, KEGG_tibs_AGE_3_trim,
#           ncol = 1, rel_heights = c(1,0.5), rel_widths = c(1, 0.8))
```
