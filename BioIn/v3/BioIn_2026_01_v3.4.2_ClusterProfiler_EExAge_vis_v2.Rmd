...
for(kp in kegg_POI) {
  ...
  prot_sums <- my_kp_prot %>% # this is needed to avoid overplotting
  ...
  my_lg_prot <- my_kp_prot %>% #LABEL LEADING GENE???
  ...
  vol_plot <- protein_comp_entrez %>%
    ...
    geom_point(data = my_kp_prot, # ORGANE POINTS
               aes(x = log2FC, y = -log10(adj.pvalue)),
               alpha = .4, size = 1.5, shape = 21,
               fill = "#E17C05FF", color = "#FCA842FF") + 
    
    geom_text_repel(data =  my_lg_prot, # TOP 5 MOST SIG ANNO
                    aes(x = log2FC, y = -log10(adj.pvalue),
                        label = gene,
                        color = log2FC > 0), # color mapping (see below)
                    max.overlaps = 30, size = 2, # gene text on points
    ...
    geom_text(data = prot_sums, # up 
              ...
              x = 1.52, y = 5.7, size = 3, color = "firebrick") + 
    geom_text(data = prot_sums, # down
              aes(label = paste0(down, " \U2193")),
              x = -1.52, y = 5.7, size = 3, color = "royalblue3") + 
...
for(rct in REACT_POI) {
  ...
  my_lg_prot <- my_rct_prot %>% #LABEL LEADING GENE???
    arrange(adj.pvalue) %>% # not quite - 5 most sig proteins
    slice_head(n = 6)
  ...
  vol_plot <- protein_comp_entrez %>%
   ...
   ggplot(aes(x = log2FC, y = -log10(adj.pvalue))) +
    ...
    # ORIGINAL PROTE COMP 
    geom_point(aes(
      fill = case_when( # conditional ass for FILL mapping
        adj.pvalue < 0.05 & log2FC > 0 ~ "up_adjp.05", # up sig .05
        adj.pvalue < 0.05 & log2FC < 0 ~ "down_adjp.05", # down sig .05
        adj.pvalue < 0.1 & log2FC > 0 ~ "up_adjp.1", # up 0.1
        adj.pvalue < 0.1 & log2FC < 0 ~ "down_adjp.1", # down .1
      ...
      alpha = case_when( # conditional ass for ALPHA mapping
        ...
        adj.pvalue < 0.1 & log2FC < 0 ~ "down_adjp.1", # down .1
        TRUE ~ "ns" )), # catch the rest
      size = 1.5, shape = 21, color = "grey50", stroke = 0.01) +
  ...
  geom_text(data = my_rct_prot_sums, # up 
            ...
            x = 1.45, y = 5.7, size = 3, color = "firebrick") + 
  geom_text(data = my_rct_prot_sums, # down
            aes(label = paste0(rct_down, " \U2193")),
            x = -1.45, y = 5.7, size = 3, color = "royalblue3") + 
    
    
    geom_text(data = path_sig_lab, # GSEA SIGNIFICANCE LABEL
              aes(label = paste0(sig_val),
                  y = 5,
                  x = ifelse(NES > 0, 1.45, -1.45), # side if neg or pos
                  color = NES > 0), # col - TRUE FALSE 
              size = 3) +
