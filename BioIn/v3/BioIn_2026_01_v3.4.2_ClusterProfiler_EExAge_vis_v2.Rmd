...
lolliplot <- function(data,
  ...
  strip <- strip_themed( # green for 1st 3 (housing) & Lavender for last 3 (age)
    background_x = elem_list_rect(fill = c(
      "#cbdec1", "#cbdec1", "#cbdec1",
      "#D5D3E4", "#D5D3E4", "#D5D3E4")))
...
for(kp in kegg_POI) {
  ...
  # strip for facet_wrap2 (conditional strip bg color)
  # strip <- strip_themed( # green for 1st 3 (housing) & Lavender for last 3 (age)
  #   background_x = elem_list_rect(fill = c(
  #     "#BDD5B0", "#cbdec1", "#cbdec1",
  #     "#C6C3DA", "#D5D3E4", "#D5D3E4")))
  ...
  vol_plot <- protein_comp_entrez %>%
    ...
    geom_text(data = prot_sums, # up 
              ...
              x = 1.52, y = 5.7, size = 2.7, color = "firebrick") + 
    geom_text(data = prot_sums, # down
              aes(label = paste0(down, " \U2193")),
              x = -1.52, y = 5.7, size = 2.7, color = "royalblue3") + 
...
for(rct in REACT_POI) {
  ...
  geom_text(data = my_rct_prot_sums, # up 
            ...
            x = 1.45, y = 5.7, size = 2.7, color = "firebrick") + 
  geom_text(data = my_rct_prot_sums, # down
            aes(label = paste0(rct_down, " \U2193")),
            x = -1.45, y = 5.7, size = 2.7, color = "royalblue3") + 
