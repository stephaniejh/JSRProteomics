...
KEGG_tibs_df <- map_dfr(KEGG_gsea_EExAge_mssv3.2.2t1_cpv3.4.2t3$kegg_results_tib,
                        ...
                        .id = "Label")  %>%
  mutate(Label_trm = paste0(gsub("^[^_]*_", "", Label)), .after = Label) %>%
  mutate(Description_2 = str_replace_all(Description, " and ", " & "), # & instead and
         .after = Description) 
...
### Bit of a mix of old & new Label vs Label_trm

```{r}
# my_levels = c("H_EE_vs_SH", # OLD LONG LABELS
#               "HxA_EE_YNG_vs_SH_YNG",
#               "HxA_EE_OLD_vs_SH_OLD",
#               "A_YNG_vs_OLD", 
#               "HxA_EE_YNG_vs_EE_OLD",
...
my_levels = c("EE_vs_SH", # NEW SHORT LABELS IN Label_trm
              "EE_YNG_vs_SH_YNG",
              "EE_OLD_vs_SH_OLD",
              "YNG_vs_OLD",
              "EE_YNG_vs_EE_OLD",
...
my_level_colors <- c("#cbdec1", "#cbdec1", "#cbdec1",
                     "#E6E6FA", "#E6E6FA", "#E6E6FA")

names(my_level_colors) <- my_levels
...
GO_sim_tibs_df_6$Label_trm <- factor(GO_sim_tibs_df_6$Label_trm,
                        levels = my_levels)
...
```{r}
# KEGG CATEGOGY ANNO + COLOR FOR STRIP
KEGG_tibs_df_6_anno <- left_join(KEGG_tibs_df_6, KEGG_cat_anno, 
                                 by = join_by(ID == gsid,
                                              Description == name)) %>%
...
lolliplot <- function(data,
  ...
  lol <- ggplot(data,
    ...
    # scale_x_discrete(labels = scales::label_wrap(width = 30)) + # break text | KEGG ver
     # scale_x_discrete(labels = scales::label_wrap(width = 40)) + # break text |
...
#### Msigdb Hallmarks - see other v3 script
...
### Trim Combo & subcategory KEGG Lolliplot
...
GO_sim_lol_6 <-   GO_sim_tibs_df_6 %>%
  ...
  # filter(NES > 0) %>%
  # filter(Label %in% c("H_EE_vs_SH","HxA_EE_YNG_vs_SH_YNG","HxA_EE_OLD_vs_SH_OLD")) %>%
  lolliplot(subtitle = "GO BP GSEA - Housing & Age +POS", legend_pos = "right")
...
kegg_POI  <- c(
  ...
  "rno01250", # Biosynthesis of nucleotide sugars
  "rno00030", "rno00230", # pentose phosphate, purine metabolism
  "rno00190", "rno00020",  # oxidati phos, TCA, 
  "rno04714", # thermogenesis
  "rno05012", "rno05016", "rno05020", # parkinsons, huntington, prion 
  "rno04137", #mitophagy
  "rno03040", "rno03082", "rno03050", # spliceo, ATPchromatre, proteasom
  "rno03010", "rno04141", "rno03013") # ribosome, protein proc in ER, nucleocytoplasmic transport
...
# ggsave("2026_04_03_KEGG_55-67_mapped_volcano_330dpi.png", dpi = 330)
```
...
REACT_POI <- unique(REACT_tibs_df_6$ID) #311
...
# 1 - PREPARE THE DATA
  ...
  vol_plot <- protein_comp_entrez %>%
    ...
    labs(subtitle = paste0("REACTOME - ", my_title)) +
...
```{r}
plot_grid(plotlist = reactome_volcano_list[1:21], ncol = 3)
...
plot_grid(kegg_volcano_list$rno04014, kegg_volcano_list$rno04024,
...
# ggsave("2026_04_04_KEGG_2_DNA_mapped_volcano_300dpi.png", dpi = 300)
```
...
# ggsave("2026_04_04_KEGG_2_RNA_mapped_volcano_300dpi.png", dpi = 300)
```
...
# ggsave("2026_04_04_KEGG_2_Protein_mapped_volcano_300dpi.png", dpi = 300)
```
...
plot_grid(kegg_volcano_list$rno04721, kegg_volcano_list$rno04082,
          ...
          ncol = 1, labels = c("a","b", "c"), label_colour = "grey10", label_size = 12)

# ggsave("2026_03_31_sig_KEGG_synaptic_mapped_volcano_330dpi.png", dpi = 330)
```
...
# ggsave("2026_04_04_KEGG_2_GABA_Neuroactive_mapped_volcano_300dpi.png", dpi = 300)
```
...
# KEGG Proteasome & Protein Processing in the ER
plot_grid(kegg_volcano_list$rno00190, kegg_volcano_list$rno00020,
          ncol = 1,
          labels = c("a","b"),
          label_colour = "grey10", label_size = 12)
...
plot_grid(kegg_volcano_list$rno05012, kegg_volcano_list$rno05022,
          kegg_volcano_list$rno05016,
          kegg_volcano_list$rno05020,
          ncol = 1,
          labels = c("a","b", "c", "d"),
          label_colour = "grey10", label_size = 12)

# ggsave("2026_04_04_KEGG_4_Neurodegenerative_mapped_volcano_300dpi.png", dpi = 300)
# dev.off()
```
