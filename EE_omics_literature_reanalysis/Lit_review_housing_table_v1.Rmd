...
tr_1_house_df <- one_house_df %>%
  ...
  filter(animal_type == "rats") %>%
  # filter(author %in% c("Angelucci", "Ickes", "Keyvani", "Novati" )) %>%
  # filter(author == "Ickes") %>%
  # select(c(animal_type:housing, length:no_per_cage_2)) %>%
...
for(cr in crosses) {
    ...
    df_1 <- tr_1_house_df %>%
      ...
      mutate(height = ifelse(is.na(height), 5, height))
    ...
    df_2 <- data.frame(
      ...
      xd = runif(n = df_1$no_per_cage_2[1],
                 min = 1, max = (df_1$length[1]) -.5),
      yd =  runif(n = df_1$no_per_cage_2[1],
                 min = 1, max = (df_1$width[1]) -.5),
      zd = runif(n = df_1$no_per_cage_2[1],
                 min = 1, max = (df_1$height[1]) -.5),
    ...
    plot_3D_cloud <- cloud(yd~zd+xd, df_2,
      ...
      par.settings = list(
        ...
        par.main.text = list(cex = .8, font = 1), # main/title
        par.sub.text = list(cex = .8, font = 1), # sub
        plot.symbol = list(cex = 3, col = paste0("",df_2$sex_col[1], "")),
...
```{r}
plot_grid(plotlist = plot_3D_list, ncol = 6)#
```
