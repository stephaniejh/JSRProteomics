...
2026_01_16 -\> moving to intro at start -\> too long -\> revisit later
...
library(lubridate)
library(ggwordcloud)
library(colorspace) # for scale_color_discrete_divergingx & display colors
...
library(ggh4x)
library(rayshader) # for plot_gg (3D plots)
library(lattice) # for 3D
library(latticeExtra) # for 3D
library(patchwork)
```
...
housing_df_3 <- housing_df_2 %>%
  ...
  mutate(cage_volume = round((length * width * height)/100, 1),
         cage_area = round((length * width)/100, 1), .after = height
           ) %>%
  ...
  mutate(no_per_cage_2 = as.numeric(recode(no_per_cage,
                                           ...
                                           "5-6" = "6")),
         .after = no_per_cage) %>%
  mutate(per_cage_vol = round(cage_volume/no_per_cage_2, 1),
         per_cage_area = round(cage_area/no_per_cage_2, 1),
         .after = no_per_cage_2)
...
<https://www.geeksforgeeks.org/r-language/change-background-color-with-plot-grid-in-r/>

```{r}
EE_wordcloud <- EE_wordcloud + theme(
  plot.background = element_rect(fill = "#BAE3A5", color = "grey10", alpha = 0.1))
...
## Cage & no. - geom_rect + fill in later

- ~~gg3D <https://htmlpreview.github.io/?https://github.com/AckerDWM/gg3D/blob/master/gg3D-vignette.html>~~

```{r}
cage_no_plot <- one_house_df %>%
  filter(!is.na(height)) %>% # rm no 3rd (height), (so full NAs & 2Ds)
  filter(animal_type == "rats") %>%
  ggplot(aes(x = length,
             y = width,
             # z = height,
             fill = housing,
  ...
  # geom_tile(aes(width = length, height = width, alpha = height)) +
  geom_rect(aes(
    xmin = 0, xmax = length,
    ymin = 0, ymax = width,
    # alpha = height
  ...
  # geom_segment(aes(x = 0, xend = height, # weird looking 
  #                  y = 0, yend = height))+ 

  # facet_wrap(~author+housing,
  #            # scales = "free",
  #            ) +
  facet_wrap2(~ author_year + housing,
               # scales = "free",
               ) +
  theme_bw()
  # theme(panel.spacing = )
...
### With Rayshader !!! works but blocky & ugly

- <https://www.tylermw.com/posts/data_visualization/3d-ggplots-with-rayshader.html>

  - <https://forum.posit.co/t/rayshader-3d-ggplots/143602>

  - <https://www.rayshader.com/reference/plot_gg.html>

```{r}
# cage_no_plot_2 <- one_house_df %>%
#   filter(animal_type == "rats") %>% # temp filter rats only
#   filter(!is.na(height)) %>% # rm no 3rd (height), (so full NAs & 2Ds)
#   mutate(height = ifelse(is.na(height), 0.1, height)) %>% #rplc missing with 1
# 
...
#   facet_wrap(~author + housing)
# # 
# plot_gg(cage_no_plot_2, multicore = TRUE,  width = 12, height = 12, scale = 100,
# background = "#afceff", shadowcolor = "limegreen")
# 
# gg1 = render_snapshot(clear = TRUE, plot = FALSE, # this plots in R viewer
#                       title_text = "RATS")

# gg2 = render_highquality() # weird looking

# render_depth(focallength=100,focus=0.72) # nope

# rayimage::plot_image(gg1) # giving error
```

### Lattice & LatticeExtra in R

- <https://stackoverflow.com/questions/65837568/r-adjust-color-gradient-for-3dbarplot-using-latticeextra-cloud-in-r>

- <https://stackoverflow.com/questions/1406202/plotting-a-wireframe-and-a-cloud-with-lattice-in-r>

```{r}
library(latticeExtra)
myPalette <- colorRampPalette(rev(c("red2","tomato2","red4","darkorange", "gold1","forestgreen")))(6)
...
```{r}
myPalette <-  c("gold1", "forestgreen", "orange")
...
test_df$x <- factor(test_df$x, levels = c("Study_1", "Study_2", "Study_3"))
test_df$z <- factor(test_df$z, levels = c("SH", "EE", "IC"))

plot_1 <- cloud(y~z+x, test_df, panel.3d.cloud=panel.3dbars, col.facet=myPalette,
             xbase=0.8, ybase=0.8,
      xlab= "Study",ylab="Housing",zlab="Height",
      scales=list(arrows=FALSE, col=1),
             par.settings = list(axis.line = list(col = "transparent")))
```

```{r}
plot_grid(plot_1, plot_1)
```

## Cage plot working with lattice!
...
```{r}
tr_1_house_df <- one_house_df %>%
  # filter(!is.na(length)) %>% # rm those without 3D
  filter(!is.na(height)) %>%
  filter(!is.na(no_per_cage_2)) %>% # rm those with missing no.
  # filter(animal_type == "mice") %>%
  filter(animal_type == "rats") %>%

  mutate(crossed_var = paste0(author_year, " ", housing)) %>%
  # mutate(box_color = case_when( # more intensely colored 
  #   housing == "EE" ~ "#A8D9A0", # EE
  #   housing == "SH" ~ "#FEE697",  # SH
  #   housing == "IC" ~ "#E6966C", # IC
  #   housing == "BE" ~ "#DD866E",  # BE
  #   housing == "NE" ~ "#C65D73",  # NE
  #   .default = "grey95"), # for NA / catch others
  # ) %>%
   mutate(box_color = case_when(
    housing == "EE" ~ "#D4ECD0", # EE
    housing == "SH" ~ "#FFF3CB",  # SH
    housing == "IC" ~ "#F3CBB6", # IC
    housing == "BE" ~ "#EEC3B7",  # BE
    housing == "NE" ~ "#E3AEB9",  # NE
    .default = "grey95"), # for NA / catch others
  ) %>%
  mutate(animal_color = case_when(
    animal_type == "pigs" ~ "rosybrown3", # pigs
    animal_type == "mice" ~ "#7570B3FF",  # mice
    animal_type == "rats" ~ "#1B9E77FF", # rats
    .default = "grey15"), # for NA / catch others
  ) %>%
...
- [https://learnr.wordpress.com/wp-content/uploads/2009/08/latbook.pdf](chrome-extension://efaidnbmnnnibpcajpcglclefindmkaj/https://learnr.wordpress.com/wp-content/uploads/2009/08/latbook.pdf){.uri}

```{r}
# see settings
current_settings <- trellis.par.get()

print(current_settings)

names(lattice.options())
lattice.getOption("panel.cloud")
str(lattice.getOption("layout.widths"), max.level = 2)
```

```{r}
# study_names <- unique(tr_1_house_df$author_year) 
# housing_names <- unique(tr_1_house_df$housing)

crosses <- unique(tr_1_house_df$crossed_var)

plot_3D_list <- vector(mode = "list")
```
...
for(cr in crosses) {
    ...
    df_1 <- tr_1_house_df %>%
      filter(crossed_var == cr) #%>%
      # mutate(height = ifelse(is.na(height), 1, height))
    ...
    df_2 <- data.frame(
      no = 1:df_1$no_per_cage_2[1],
      # xd = rnorm(n=1:df_1$no_per_cage_2[1], # old version
      #            mean = df_1$length[1]/2,
      #            sd = df_1$length[1]/7),
      # xd = sample(1:df_1$length[1],
      #            df_1$no_per_cage_2[1],
      #            replace = FALSE),
      xd = runif(n = df_1$no_per_cage_2[1],
                 min = .1, max = (df_1$length[1]) -.1),
      yd =  runif(n = df_1$no_per_cage_2[1],
                 min = .1, max = (df_1$width[1]) -.1),
      zd = runif(n = df_1$no_per_cage_2[1],
                 min = .1, max = (df_1$height[1]) -.1),
      
                 h_name = df_1$housing[1],
                 s_name = df_1$author_year[1],
                 height = df_1$height[1],
                 width = df_1$width[1],
                 length = df_1$length[1],
                 box_col = df_1$box_color[1],
                 sex_col = df_1$sex_color[1],
                 animal_col = df_1$animal_color[1],
                 cage_vol = df_1$cage_volume[1],
                 no_in_cage = df_1$no_per_cage_2[1],
                 p_cage_vol = df_1$per_cage_vol[1],
                 stringsAsFactors = TRUE)
    
    # MAKE 3D CLOUD PLOT
    plot_3D_cloud <- cloud(yd~zd+xd, df_2,
      # panel.3d.cloud=panel.3dpolygon,
      # xbase=0.8, ybase=0.8,# does this make it smaller? 
      
      xlab= paste0("",df_2$length[1], ""),
      ylab= paste0("",df_2$width[1], ""),
      zlab= paste0("",df_2$height[1], ""),
      xlim = c(0, df_2$length[1]),
      ylim = c(0, df_2$width[1]),
      zlim = c(0, df_2$height[1]),
      aspect = c(df_2$width[1]/df_2$length[1],
                 df_2$height[1]/df_2$length[1]),
      zoom = df_2$length[1]/135,
      main = paste0(df_2$s_name[1], " - ", df_2$h_name[1]), 
      # xlab.top = paste0("", ,""), # additional title @ top! ,
      xlab.top = list(label = paste0("",
                                     # df_2$p_cage_vol[1], " cm\u00B3/mouse (", # mouse
                                     df_2$p_cage_vol[1], " cm\u00B3/rat (", # rat
                                     df_2$no_in_cage[1], " per cage)"
                                     ),
                      cex = 0.65, col = "grey30", font = 3,
                      y = 0, just = "bottom"), # moves down?
      scales=list(arrows=TRUE, col= paste0("",df_2$box_col[1], "") # OMG THIS WORKS!!!
                  ), 
      par.settings = list(
        axis.line = list(col = paste0("", df_2$animal_col[1], "")), # border
        axis.text = list(cex = 0.5,col = "grey20"),
        par.xlab.text = list(cex = .5, col = "grey20"),
        par.ylab.text = list(cex = .5, col = "grey20"),
        par.zlab.text = list(cex = .5, col = "grey20"),
        par.main.text = list(cex = .75, font = 1), # main/title

        par.sub.text = list(cex = .8, font = 1), # sub
        plot.symbol = list(cex = 2, col = paste0("",df_2$sex_col[1], "")),
        
        panel.background = list(col = paste0("",df_2$box_col[1], ""), alpha = 0.1),
        
        box.rectangle = list(fill = paste0("",df_2$sex_col[1], "")), # WORKS!!!
        box.3d = list(col = paste0("",df_2$sex_col[1], "")),
        
        # layout.widths = list(panel = c(2,1.5)) #4:3 ratio WIDE STROTCH
        
        layout.widths = list(left.padding = 0, right.padding = 0), # rm padding
          layout.heights = list(top.padding = 1, bottom.padding = -1,
                                main = -0.5, 
                                main.key.padding = 1.5)
...
```{r}
plot_grid(plotlist = plot_3D_list[c(1:2,6:11)], ncol = 2)#

# plot_grid(plotlist = plot_3D_list, ncol = 2)#
```

```{r}
(plot_3D_list$`Angelucci 2009 EE` + plot_3D_list$`Angelucci 2009 SH` )
```

```{r}
# df_1 <- tr_1_house_df %>%
#       filter(author_year == "Angelucci 2009") %>%
#       filter(housing == "EE") 
# 
#   
# df_2 <- data.frame(no = 1:df_1$no_per_cage_2,
#                    xd = rnorm(n=1:df_1$no_per_cage_2, mean = df_1$length/2, sd = df_1$length/5),
#                    yd = rnorm(n=1:df_1$no_per_cage_2, mean = df_1$width/2, sd = df_1$width/5),
#                    zd = rnorm(n=1:df_1$no_per_cage_2, mean = df_1$height/2, sd = df_1$height/5),
#                    h_name = df_1$housing,
#                    s_name = df_1$author_year,
#                    height = df_1$height,
#                    width = df_1$width,
#                    length = df_1$length,
#                    stringsAsFactors = TRUE)
...
#              xbase=0.8, ybase=0.8,
#       xlab= "Study",ylab="Housing",zlab="Height",
#       xlim = c(0, df_2$length[1]), ylim = c(0, df_2$width[1]), zlim = c(0, df_2$height[1]),
#       scales=list(arrows=FALSE, col=1),
#              par.settings = list(axis.line = list(col = "transparent")))
# 
# # wireframe(width~length+height, df_2, drape = TRUE)
```

<https://stackoverflow.com/questions/1406202/plotting-a-wireframe-and-a-cloud-with-lattice-in-r>

```{r}
# plotplots <- unlist(plot_3D_list)
```
...
opt_stylize(style = 1, color = 'pink', add_row_striping = TRUE)
```
