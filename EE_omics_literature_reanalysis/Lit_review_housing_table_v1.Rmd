...
timeline_plot <- WT_ctrl_EE_only %>%
  ...
  mutate(author_year_hc = )
  
  # ORDERING BY YEAR & HOUSING DURATION! 
  # this is a new composite variable for reorder() function below which only takes 1 var
  ...
  mutate(study_yr_by_dur = (year + (housing_duration_days/1000))) %>% 
  
  
  
  # COMPUTE VAR for element_markdown() (for custom axis text colors)
  mutate(color = c("#A80062", "#1B5988", "grey15"),
         
         
         names = glue("<i style='color:{color}'>**{author_name}**</i> ({housing_condition})"),
         names = reorder(names, animal_sex)
         ) %>%
