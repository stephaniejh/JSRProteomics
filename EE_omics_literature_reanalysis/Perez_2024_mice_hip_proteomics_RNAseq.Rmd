...
lit_review_transcriptomics <- RNA_df_map_rat_combined %>%
  rename(orig_pvalue = pvalue) %>% #2026_07 - rename because orig RNA file has both pvalue & padj cols
  mutate(
    Year = 2024,
    Author = 'Perez',
    Animal = 'Mice',
...
# write_xlsx(split_RNA_df_map_rat, # OLD VERSION 
#            path = "output/2025_12_27_Perez_2024_transcriptomics_lit_review_all_v1.xlsx")

# # NEW VERSION 2026_07_04
# write_xlsx(split_RNA_df_map_rat, # NEW FIXED PVALUE COL! 
#            path = "output/2026_07_04_Perez_2024_transcriptomics_lit_review_all_v2.xlsx")
```
...
## Quick keyword analysis 2025_12_31
...
### Finding neuro related words + others of interest

```{r}
neuro_words <- c("brain", "synaptic", "synapse", "neuro", "synap", "axon",
                 "dendritic", "dendrite", "neural", "nerve", "nervous",
                 "glutamate", "spinal", "cortical", "cortex", "cereb",
                 "action potential", "membrane potential", "electrochemical",
                 "serotonin", "dopamine", "noradrenalin", "ionotropic", "chloride",
                 "sodium", "potassium", "NMDA", "AMPA",
                 "cytoskeleton", "cell projection", "GTP", "Mitochondrion", 
                 "respiratory chain", "oxidative phosphorylation",
                 "Neurotransmitter", "Ribosomal protein", 
                 "signal transduction", "signaling pathway")

neuro_pattern <- paste(neuro_words, collapse = "|") # |OR
```

```{r}
key_annos_neuro <- key_annos %>%
  filter(str_detect(Definition, neuro_pattern) | str_detect(Name, neuro_pattern))
```

```{r fig.height=8, fig.width=11}
keyplot_prot_3 <- keywords_counts_prot %>%
  filter(!hashtags == "Cytoplasm") %>% 
  filter(str_detect(Definition, neuro_pattern) | str_detect(hashtags, neuro_pattern)) %>%
  ggplot(aes(x = hashtags,
             y = percent,
             color = In_EE,
             fill = Category)) +
  ...
  facet_nested(proteomics_id~Category, 
             ...
             space = 'free') +
  labs(x = "Uniprot Keyword", y = "Count",
       title = "Perez 2024 - Neuro-related Keywords in Significant Proteins",
       subtitle = "") +
  scale_fill_brewer(palette = "Spectral") +
  scale_color_manual(values = c("royalblue4", "firebrick3")) +
  theme_bw() +
...
```{r fig.height=8, fig.width=11}
keyplot_RNA_3 <- keywords_counts_RNA %>%
   filter(!hashtags == "Cytoplasm") %>% 
  filter(str_detect(Definition, neuro_pattern) | str_detect(hashtags, neuro_pattern)) %>%
  ggplot(aes(x = hashtags,
             y = percent,
  ...
  facet_nested(transcriptomics_id~Category, 
             ...
             space = 'free') +
  labs(x = "Uniprot Keyword", y = "Count",
       title = "Perez 2024 - Neruo-related Keywords in Significant Proteins",
       subtitle = "") +
  scale_fill_brewer(palette = "Spectral") +
  scale_color_manual(values = c("royalblue4", "firebrick3")) +
  theme_bw() +
