...
Make proteomics lit review file
...
# prev code for mapping 
# rename(ENS_GENEID = GENEID) %>%
# make new col remove all letters but keeping 11 digit identifier (otherwise ENSMUSG)
# mutate(ENS_GENEID_NUM = paste0(gsub("[[:alpha:]]", "", ENS_GENEID))) %>%
# mutate(ENS_GENEID_T = paste0("ENSMUST", ENS_GENEID_NUM)) # make ENSMUST000094324
```

```{r}
# mRNA_check_1 <- mRNA_df %>%
#   filter(nchar(ENS_GENEID_NUM) > 12) # double check there isn't any > 11 digits
```

```{r}
ensembl_to_search <- as_tibble(unique(mRNA_df$ENSEMBL))

# write_csv(ensembl_to_search, "output/Perez_2024_mRNA_ensembl_to_search.csv")
```

```{r}
ensembl_2_uniprot <- read_excel(
  "input/Perez_2024_mRNA_idmapping_2025_12_27_ensembl_2_uniprot_15703_map_to_44296_res_4132_unmap.xlsx")

ensembl_2_uniprot <- ensembl_2_uniprot %>%
  mutate(existence_ranking = case_when(
    `Protein existence` == "Evidence at protein level" ~ 5,
    `Protein existence` == "Evidence at transcript level" ~ 4,
    `Protein existence` == "Inferred from homology" ~ 3,
    `Protein existence` == "Predicted" ~ 2, 
...
```{r}
# map mRNA df to uniprot df 
mRNA_df_map_mouse_1 <- left_join(mRNA_df, ensembl_2_uniprot,
                                 by = join_by(ENSEMBL == From))
```

```{r}
# checks before next step 
dim(mRNA_df_map_mouse_1) # 145284 x 51
sum(is.na(mRNA_df_map_mouse_1$Entry)) # 12396 

# to check for grouping 
length(unique(mRNA_df_map_mouse_1$transcriptomics_id)) # 3 
length(unique(mRNA_df_map_mouse_1$ENSEMBL)) # 19835
length(unique(mRNA_df_map_mouse_1$SYMBOL)) # 19642
length(unique(mRNA_df_map_mouse_1$`Gene Names (primary)`)) # 15804
length(unique(mRNA_df_map_mouse_1$Entry)) # unique ## 44114
```

```{r}
mRNA_df_map_mouse_2 <- mRNA_df_map_mouse_1 %>% # 145284
  group_by(transcriptomics_id, ENSEMBL) %>% 
  filter(if(any(Reviewed == "reviewed", na.rm = TRUE)) {
    Reviewed == "reviewed"
  } else{TRUE}) %>% # 73755
  slice_max(Annotation, n = 1) %>% #65559 
  slice_max(existence_ranking, n =1) %>% # 64347
  slice_max(occupied, n = 1, with_ties = FALSE) %>% # 60057 (no ties) | 59505
  ungroup()
```

```{r}
mRNA_df_map_mouse_2_na <- mRNA_df_map_mouse_2 %>%
  filter(is.na(Entry)) %>% # 12396 unmapped
  select(transcriptomics_id:SYMBOL)
```

```{r}
mRNA_df_map_mouse_2_mapped <- mRNA_df_map_mouse_2 %>%
  filter(!is.na(Entry)) 
```

Map remaining by primary gene symbol

```{r}
mRNA_df_map_mouse_3 <- left_join(mRNA_df_map_mouse_2_na, ensembl_2_uniprot, 
                                 by = join_by(SYMBOL == `Gene Names (primary)`))
```

```{r}
sum(is.na(mRNA_df_map_mouse_3$Entry)) 
```

```{r}
mRNA_df_map_mouse_3 <- left_join(mRNA_df_map_mouse_2_na, ensembl_2_uniprot, 
                                by = join_by(SYMBOL == `Gene Names (primary)`)) %>%
  group_by(transcriptomics_id, ENSEMBL) %>% # 57321
  filter(if(any(Reviewed == "reviewed", na.rm = TRUE)) {
    Reviewed == "reviewed"
  } else{TRUE}) %>% # 57264
  slice_max(Annotation, n = 1) %>% #12900
  slice_max(existence_ranking, n =1) %>% # 12396
  slice_max(occupied, n = 1, with_ties = FALSE) %>% # 12396? | same with no ties?
  ungroup()
```

```{r}
mRNA_df_map_mouse_3_mapped <- mRNA_df_map_mouse_3 %>%
  filter(!is.na(Entry)) 
```

```{r}
# lots of leftover unmapped are tRNA, Rik, etc (non protein stuff?)
mRNA_df_map_mouse_3_NA <- mRNA_df_map_mouse_3 %>%
  filter(is.na(Entry)) 
```
...
```{r}
RNA_df_map_mouse_combined <- bind_rows(list(
  by_ensembl_2_uniprot = mRNA_df_map_mouse_2_mapped,
  by_symbol_2_p_gene_name = mRNA_df_map_mouse_3),
  .id = "mouse_map_method_df")

```

```{r}
# barely any sig mRNA! 345 @ < 0.05 | or 190 < 0.05 
RNA_df_map_mouse_combined_sig <- RNA_df_map_mouse_combined %>%
  filter(padj <= 0.05) 
```

```{r}
RNA_df_map_mouse_combined_sig |> 
  # dplyr::ungroup() |>
  # dplyr:: filter(Condition == "all") %>%
  #dplyr::select(-(c(`Av. Ratio`, ))) |>
  # dplyr::select(c(Condition, In_EE, Gene_name, Entry, Entry.Name,
  #                 Protein.names, Fold_change, adj.P.Val)) |>
  # dplyr::arrange(desc(In_EE), adj.P.Val, Fold_change) |>
  dplyr::select(c(transcriptomics_id, SYMBOL, Entry, `Entry Name`,
                  `Protein names`, log2FoldChange, padj)) |>
  gt(
  ...
  fmt_number(decimals = 3) |>
    tab_header(
    title = "All - Perez 2024 - s - Hippocampus (Mice) (adj.p <=0.05 only)",
    subtitle = "2025_12_17 - lit_review_proteomics_sig - (EE vs SH - by all)"
    ) |>
    tab_options(table.font.size=11.5,
                stub.font.weight = "bold",
...
```{r}
RNA_df_map_rat_1 <- left_join(mRNA_df, rat_ref,
                              by = join_by(SYMBOL == Gene.Names..primary.))
```
...
```{r}
RNA_df_map_rat_1_NA <- RNA_df_map_rat_1 %>%
  filter(is.na(Entry)) %>%
  select(transcriptomics_id:SYMBOL)
```

```{r}
RNA_df_map_rat_2 <- left_join(RNA_df_map_rat_1_NA, rat_ref_split,
                              by = join_by(SYMBOL == split_synonyms))
```

```{r}
RNA_df_map_rat_2_NA <- RNA_df_map_rat_2 %>%
  filter(is.na(Entry))
```
...
```{r}
RNA_df_map_rat_combined_sig <- RNA_df_map_rat_combined %>%
  filter(padj <= 0.05)
```

```{r}
RNA_df_map_rat_combined_sig |> 
  # dplyr::ungroup() |>
  # dplyr:: filter(Condition == "all") %>%
  #dplyr::select(-(c(`Av. Ratio`, ))) |>
  # dplyr::select(c(Condition, In_EE, Gene_name, Entry, Entry.Name,
  ...
  fmt_number(decimals = 3) |>
    tab_header(
    title = "All - Perez 2024 - s - Hippocampus (Mice) (adj.p <=0.05 only)",
    subtitle = "2025_12_27 - lit_review_RNA_sig - (EE vs SH - by all)"
    ) |>
    tab_options(table.font.size=11.5,
                stub.font.weight = "bold",
                row_group.font.weight = "bold",
