...
prot_df_map_rat_2 <- prot_df_map_rat_1 %>% #24513 | 337629
  ...
  mutate(id_priority = case_when(
    # does not help!!! 
    # !is.na(GeneID.m) & !is.na(GeneID) & !is.na(Ensembl.m) & !is.na(Ensembl) ~ 10,
    # !is.na(GeneID.m) & !is.na(GeneID) & !is.na(Ensembl) ~ 8,
    # !is.na(Gene.Ontology.IDs) & !is.na(GeneID) & !is.na(Ensembl) & !is.na(KEGG) ~ 6,
    # !is.na(GeneID.m) & !is.na(GeneID) & !is.na(Ensembl) & !is.na(KEGG) ~ 6,
    # !is.na(Reactome) & !is.na(GeneID) & !is.na(Ensembl) & !is.na(KEGG) ~ 6, 
    #!is.na(Function..CC.) & !is.na(GeneID) & !is.na(Ensembl) & !is.na(KEGG) ~ 6, 
    #!is.na(STRING) & !is.na(GeneID) & !is.na(Ensembl) & !is.na(KEGG) ~ 7, 
  ...
  slice_max(id_priority, n = 1) %>%
  dplyr::select(-id_priority) # rm col otherwise will have uneven lengths for binding 
...
prot_df_map_rat_2_dups <- prot_df_map_rat_2 %>%
  ...
  filter(n > 1) # 216 with duplicate entries
...
length(unique(prot_df_map_rat_2$Entry)) # 2242
```
...
Recovered Sh3bgrl, Tceal3, Mtco2, Mtnd1, Mtnd5, Eif2s3x (gene synonyms that match mouse primary)
...
lit_review <- prot_df_map_rat_combined %>%
  mutate(
    Year = 2024,
    Author = 'Perez',
    Animal = 'Mice',
