...
prot_df_map_rat_2 <- prot_df_map_rat_1 %>% #24513 | 337629
  ...
  filter(if(any(Reviewed == "reviewed", na.rm = TRUE)) {
   ...
   } else {TRUE}) %>% #10278|44907 - if none, keep unreviewed ones, & then filter below
    # 3
...
prot_df_map_rat_4 <- left_join(prot_df_map_rat_3_na, rat_other,
  ...
  slice_max(occupied, n = 1) # goes from 90 entries to 87 - only drops dup of Grcc10
...
```{r}
prot_df_map_rat_combined <- bind_rows(list(by_ref_primary_gene = prot_df_map_rat_2,
...
lit_review_proteomics <- prot_df_map_rat_combined %>%
  mutate(
    ...
    Mod = " ",
    Mod_type = " ",
    Gene_name = `gene.names.primary.m`, # genename used to map mouse primary to rat gene
    Comparison = 2,
    In_EE = dir,
...
NOTE: Tested a little bit, but commented out to knit, continue later.
...
```{r}
# # split cells with multiple ensembl id entries so 1 entry per cell
# # trim the dots at the end due to 
# mouse_ref_4_mRNA <- mouse_ref %>%
#   mutate(ensembl.split.m = str_split(Ensembl, ";"),
#          .after = Ensembl) %>%
#   tidyr::unnest(ensembl.split.m) %>% # from 54864 to 121602
#   mutate(ensembl_short = paste0(gsub("\\..*", "", ensembl.split.m)),
#          .after = ensembl.split.m) %>%
#   mutate(ensembl_short_no = paste0(gsub("[[:alpha:]]", "", ensembl_short)),
#          .after = ensembl_short)
```

```{r}
# mRNA_df_mouse_1 <- left_join(mRNA_df, mouse_ref_4_mRNA,
#                            by = join_by(SYMBOL == Gene.Names..primary.)) 
# 
# mRNA_df_mouse_1_mapped <- mRNA_df_mouse_1 %>%
#   filter(!is.na(Entry))
# 
# mRNA_df_mouse_1_na <- mRNA_df_mouse_1 %>%
#   filter(is.na(Entry)) %>%
#   select(transcriptomics_id:ENS_GENEID_NUM)
```

```{r}
# mRNA_df_mouse_2 <- left_join(mRNA_df_mouse_1_na, mouse_ref_4_mRNA,
#                              by = join_by(ENS_GENEID_NUM == ensembl_short_no))
# 
# sum(!is.na(mRNA_df_mouse_2$Entry)) #4587 mapped & 9006 unmapped
# 
# mRNA_df_mouse_2_mapped <- mRNA_df_mouse_2 %>%
#   filter(!is.na(Entry))
# 
# mRNA_df_mouse_2_na<- mRNA_df_mouse_2 %>%
#   filter(is.na(Entry)) %>%
#   select(transcriptomics_id:ENS_GENEID_NUM)
```
...
```{r}
# # from 59505 to...
# mRNA_df_adj_sig <- mRNA_df %>%
#   filter(padj <= 0.05) # huh barely any sig @ adjp <=0.05 190 (345 if 0.1 instead)
# 
# 
# mRNA_df_sig <- mRNA_df %>%
#   filter(pvalue <= 0.05) # if using normal p value 3942 significant... 

```

```{r}
# length(intersect(mRNA_df$SYMBOL, prot_df$SYMBOL)) # 2227 intersect. 
```
