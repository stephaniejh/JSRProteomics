...
*Contents (Map, Lit review table, gt table, lollipop plot)*
...
keywords_counts <- keywords_df %>%
  group_by(In_EE) %>%
  filter(hashtags != "Reference proteome") %>%  # ignore if it says ref proteome | 44
...
keyplot <- keywords_counts %>%
  ...
  # filter(n > 3) %>%
  ggplot(aes(x = reorder(hashtags, n), y = n, fill = reorder(hashtags, n))) +
  geom_col(color = "grey10", alpha = 0.7) +
  facet_wrap(~In_EE, scales = "free", ncol = 2 ) +
  labs(x = "Uniprot Keyword", y = "Count",
       title = "Top 11 Keywords") +
