...
mapped_psum <- left_join(mapped_psum_v3.2.2_EExAge,
                                       ...
                                       by = join_by(
                                         ...
                                         Mixture == Mixture,
                                         Channel == Channel)) %>%
  rename(Condition = Condition.x , Housing_c_anno = Condition.y) %>%
...
unique_check <- mapped_comp %>% 
  select(Label, sig_no_down_lab, sig_no_up_lab) %>%
...
sig_volcano_sum <- mapped_comp %>%
  select(Label, sig_no_up_lab, sig_no_down_lab) %>%
  unique()
...
```{r}
# # base R subset & make list 
EE_list_up <- as.vector(subset(mapped_comp,
                  Label == "H_EE_vs_SH" & adj.pvalue <.05 & log2FC > 0,
                  select = Protein))


EE_list_down <- as.vector(subset(mapped_comp,
                  Label == "H_EE_vs_SH" & adj.pvalue <.05 & log2FC < 0,
                  select = Protein))
```
...
```{r}
# shows variation!!! 
psum_EE_plot_test <- mapped_psum %>%
  
  filter(Protein %in% EE_list_down$Protein) %>%
  # filter(gene == "Penk") %>%
  filter(gene == "Hmgcs1") %>%
  #sex is missing from anno?
  mutate(Sex = ifelse(Mixture == "B1", "M",
                      ifelse(Mixture == "B2", "M", "F"))) %>%
  mutate(Age = ifelse(str_detect(Condition, "OLD"), "OLD", "YNG")) %>%
  
  ggplot(aes(x = Channel, y = Abundance,
             fill = Housing
             # alpha = Age
             )) +
  geom_boxplot(width = 0.5, outlier.shape = NA, alpha= 0.6) +
  geom_point(aes(color = TechRepMixture), shape = 1) +
  scale_fill_manual(values = c("#66A61EFF", "#E6AB02FF")) +
  scale_color_manual(values = c("#3288BDFF", "#CA562CFF", "#5E4FA2FF")) +
  scale_alpha_manual(values = c(0.6,0.7)) +
  
  facet_nested(~Mixture+Cohort,
               scales = "free_x", 
               space = "free") + 
  labs(title = "HMGCOA Syntase 1 ")
  # facet_wrap()
  boxplot_theme +
  theme(
...
boxplot_function <- function(data, x , y, # function inputs
  ...
  p <- ggplot(data, aes({{x}},{{y}},
    ...
    geom_boxplot(aes(fill = {{fill}}),
                 width = 0.55, size = 0.7, fatten = 1, linewidth = 0.2,
...
mapped_comp |> 
  ...
  dplyr::select(c(Label,Direction, gene, Protein, description, 
                  log2FC, adj.pvalue, pvalue, database)) |>
