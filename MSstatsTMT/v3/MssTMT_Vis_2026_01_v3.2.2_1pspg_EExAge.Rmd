...
# QC Plots - TO DO - MOVE TO OTHER SCRIPT!
...
merged_pca_cc <- merge(prots_anno, pca_scores_cc, by = 0) # row names set 4 by = 0 
...
big_volcano <- mapped_comp %>%
  ...
  # POINTS
  geom_jitter(aes(color = case_when( # conditional ass for color mapping
  ...
  scale_color_manual(values = c("firebrick", # coloring
                     ...
                     labels = c("\U2191 (adj.p <.05)",
                                ...
                                "\U2191 (adj.p <.1)",
                                "\U2193 (adj.p <.1)")) +
  scale_alpha(range = c(1,.05), guide = "none") + #  GO TRIM VERSION so matches others
  ...
  # TESTING WITH COLOR GG REPEL - NOT SURE IF I LIKE BETTER?
  # ggnewscale::new_scale_color() + # new scale color
  # geom_text_repel(
  ...
  #   values = c(`TRUE` = "firebrick4", `FALSE` = "royalblue4")) +
  # 
  ...
  theme(
    ...
    legend.title = element_text(color = "grey10", size = 8),
    legend.text = element_text(color = "grey10", size = 7),
    ...
    legend.margin = margin(-15,0,-10,0),  # space between plot & legend
...
borg_merge <- borg_merge  %>%
  ...
  mutate(EE_vs_SH = case_when(
    ...
    pvalue < 0.05 & log2FC > .04 ~ "\u2191", # up sig .05
    pvalue < 0.05 & log2FC < -.04 ~ "\u2193", # down sig .05
    # adj.pvalue < 0.7 & log2FC > .04 ~ "\u2191", # up sig .05
    # adj.pvalue < 0.7 & log2FC < -.04 ~ "\u2193", # down sig .05
    .default = " " # ‐
  )) %>%
...
```{r}
ggsave("2025_03_02_Borgmeyer_redo_table.png", dpi = 150)
```
...
EE_syn_check <- mapped_psum %>%
  ...
  ggplot(aes(x = interaction(Age, Housing, 
             ...
             y = Abundance,
             fill = Housing, alpha = Age, color = Sex)) +
  geom_boxplot(width = 0.45, size = 0.7, median.linewidth = .4, linewidth = 0.2,
                 position = position_dodge(width = 0.7),
                 outlier.shape = NA,
                 color = "grey10") +
  stat_summary(fun=mean, geom="point", position = position_dodge(width = 0.7),
               shape=4, size = 1, stroke = 0.2, color ="grey10") +
  geom_point(size = .2, shape = 1, stroke = .5,
  ...
  # guides(y.sec = 'axis') + # adds 2nd y axis (right side)
  facet_wrap(~gene, ncol = 9, scale = "free_y") +
  my_theme +
  theme(
