...
EE_Volcano <- mapped_comp %>%
  ...
  scale_color_manual(values = c("firebrick", # coloring
                     ...
                     breaks = c("up_adjp.05",
                                ...
                                "up_adjp.1",
                                "down_adjp.1"),
                     labels = c("\U2191 (adj.p <.05)", \U2191  \U2193
                                "\U2193 (adj.p <.05)",
                                "\U2191 (adj.p <.1)",
                                "\U2193 (adj.p <.1)")) +
...
EE_facet_up_plot <- mapped_psum %>%
  ...
  geom_boxplot(width = 0.45, size = 0.7, fatten = 1, linewidth = 0.2,
                 position = position_dodge(width = 0.7),
                 outlier.shape = NA,
                 color = "grey10") +
  stat_summary(fun=mean, geom="point", position = position_dodge(width = 0.7),
...
EE_facet_down_plot <- mapped_psum %>%
  ...
  ggplot(aes(x = interaction(Age, Housing, 
             ...
             y = Abundance,
             fill = Housing, alpha = Age)) +
  geom_boxplot(width = 0.5, size = 0.6, fatten = 1, linewidth = 0.2,
                 position = position_dodge(width = 0.7),
                 outlier.shape = NA,
                 color = "grey10") +
  stat_summary(fun=mean, geom="point", position = position_dodge(width = 0.7),
...
Age_facet_up_plot <- mapped_psum %>%
  ...
  ggplot(aes(x = interaction(Age, Housing, 
             ...
             y = Abundance,
             fill = Housing, alpha = Age)) +
  geom_boxplot(width = 0.5, size = 0.7, fatten = 1, linewidth = 0.2,
                 position = position_dodge(width = 0.7),
                 outlier.shape = NA,
                 color = "grey10") +
  stat_summary(fun=mean, geom="point", position = position_dodge(width = 0.7),
...
me_and_hu_gt <- EE_Hu_overlap_my_df %>%
  dplyr::ungroup() %>%
  dplyr::select(c(Animal, Region, Exp_type, Method, Used_name,
                 Gene_name, Full_name, Label,
...
```{r}
borg_trm <- borg %>%
  group_by(Used_name, Uniprot_rat_accession) %>%
  dplyr::slice(1) %>%  # rm 3 209 tp 206
  dplyr::select(Year:misc)
```
...
# 5 proteins - Hcn2 K/NA, Dhrs4, Fascin, neuritin, 	Rho GTPase-act prot 39
intersect(borg_trm$Uniprot_rat_accession, EE_main_only$Protein)
```
