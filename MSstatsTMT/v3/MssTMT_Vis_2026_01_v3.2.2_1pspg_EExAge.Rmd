...
EE_facet_up_plot <- mapped_psum %>%
  ...
  mutate(Condition = forcats::fct_reorder(Condition, PND)) %>% # reoder by PND (num)
  ...
  geom_boxplot(width = 0.55, size = 0.7, fatten = 1, linewidth = 0.2,
                 position = position_dodge(width = 0.7),
                 outlier.shape = NA,
                 color = "grey10") +
  stat_summary(fun=mean, geom="point", position = position_dodge(width = 0.7),
               shape=4, size = .8, stroke = 0.2, color ="grey10") +
  geom_point(color = "grey10", size = 1, shape = 1, stroke = .2, 
      position = position_jitterdodge(dodge.width = 0.6, jitter.width = 0.4)) +
  scale_fill_manual(values = c("#66A61EFF", "#E6AB02FF")) + # housing colors 
