...
# MSstatsTMT_2025_12_v3.2.1_1pspg EE x Age analysis (after removing outlier TMT 129N B1)

**2026_04_16 Note - TLDR:** More or less the same - the year/batch differences are the biggest drivers of differences (i.e. 2022, with better pipette tips & more consistency). Otherwise all variants are somewhat similar – therefore using this version for the PhD write up but will revisit for publication in the future.
...
- MSstatsTMT_2026_03_v3.4.1_EExAge_55_contrasts_redo.rmd -\> "covariate" version where contrasts are specified as shown in page 14 of Kohler et al., 2024 (<https://doi.org/10.1038/s41596-024-01000-3>) & also in Olga Vitek's presentation (<https://www.youtube.com/watch?v=dwuW_U7AEX0&t=1610s>). This doesn't really do much since the groups are split evenly anyway. EE vs SH is the same 16 proteins (same order and also same ranked genelist). YNG vs OLD has 3 more significant proteins going from 226 to 229.

***2025_12_31 - This version runs with 55 outlier rm (see MSstatsTMT_2025_12_v3.2.1_sp.Rmd for notes)***
...
library(purrr) # for compact, list_rbind
...
quant.msstats <- proteinSummarization(input.pd,
                                      method="msstats", #Tukey median polish w/ imputati
                                      global_norm=TRUE,
                                      reference_norm=TRUE,
...
## 2026_06_05 - saved output from 

#### 1- Intital test

```{r}
# can do vector or pre allocate with length of fitted models?
models_list <- vector(mode = "list", length(test.contrasts.msstats$FittedModel))
names(models_list) <- names(test.contrasts.msstats$FittedModel) #?
```

```{r}
for (m in seq_along(test.contrasts.msstats$FittedModel)) {
  models_list[[m]] <- summary(test.contrasts.msstats$FittedModel[[m]])
}
```

```{r}
base_table <- as.data.frame(coef(summary(test.contrasts.msstats$FittedModel$A0A096MJ99)))

#test.contrasts.msstats$FittedModel$A0A096MJ99
# test.contrasts.msstats$FittedModel$A0A0G2JWQ8 # skipped? | way emptier?
```

```{r}
# broom does not work & broom.mixed can't be installed? 
```

#### 2 - Better but still noob

```{r}
tested_prot_list <- names(test.contrasts.msstats$FittedModel)

# Pre-allocate with names
model_vec <- vector(mode = "list", length = length(tested_prot_list))
names(model_vec) <- tested_prot_list

for (m in tested_prot_list) {
  model_vec[[m]] <- coef(summary(test.contrasts.msstats$FittedModel[[m]])) 
}
```

```{r}
# broom does not work & broom.mixed can't be installed? 
sum(!sapply(model_vec, is.null))   # filled
sum(sapply(model_vec, is.null))    # still NULL / failed
```

```{r}
# use purrr compact to just keep non empties & then list_rbind
model_vec_2 <- compact(model_vec)
```

```{r}
# Convert all elements to data frames first
model_vec <- lapply(model_vec, function(x) {
  if (is.null(x)) return(NULL)
  as.data.frame(x)
})

# Then remove NULLs and bind
model_vec <- compact(model_vec)
model_vec_df <- list_rbind(model_vec, names_to = "Protein_Accession")
```

```{r}
model_vec_df <- rownames_to_column(model_vec_df, var = "id")
```

### 3 - More elegant solution (saved ver 2026_06_05)
...
for (m in tested_prot_list) {
  ...
  model <- test.contrasts.msstats$FittedModel[[m]]
  
  if (!inherits(model, c("lmerModLmerTest", "lm"))) next
  
  fixed_ef <- as.data.frame(coef(summary(model)))
  fixed_ef$term       <- rownames(fixed_ef)
  fixed_ef$protein    <- m
  fixed_ef$model_type <- class(model)[1]
  rownames(fixed_ef)  <- NULL
  
  model_vec[[m]] <- fixed_ef
}

all_coefs <- bind_rows(Filter(Negate(is.null), model_vec))
```

2026_06_05 - Merge the coefficients from MSstatsTMT models to the Uniprot FASTA

2025_12_31_1pspg_test_contrasts_msstats_v3.2.2_t1_EExAge.rda + UP000002494_10116_Rat_Ref_Proteome_22403_1pspg_2025_04_UPfastadf.csv

```{r}
# joined with mapped df
mapped_all_coefs <- left_join(all_coefs, v3_1pspg_PD, 
                         by = join_by(protein == unique_id)) # becomes 32192 x 18 (8048 proteins x 4)
```

```{r}
# save! 
# write_excel_csv(mapped_all_coefs,
#               "output/MSstatsTMT/comp/model_outputs/2026_06_05_1pspg_msstats_v3.2.2_t1_EExAge_mapped_model_coef.csv")

# save(mapped_all_coefs,
#    file = "output/MSstatsTMT/comp/model_outputs/2026_06_05_1pspg_msstats_v3.2.2_t1_EExAge_mapped_model_coef.rda")
```
