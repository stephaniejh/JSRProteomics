...
# data("/input/MSstatsTMT/psum/2025_09_12_rp_1pspg_Protein_Summarization_v3.1.5_t8.rda")
...
#quick check 
unique(prots$rcbr)
length(unique(psum$rcbr)) # 168 = 16 x 4 x 3
...
prot_mat <- prots %>%
  ...
  ##  OR 
  pivot_wider(names_from = Protein, # speads id Protein as cols
              values_from = Abundance) %>% # keeps rcbr as rows
...
rownames(prot_mat) <- prot_mat$rcbr
...
# rownames(prots_anno) <- prots_anno$rcbr
```

```{r}
# Do this way instead. for some reason the doing it directly results in 28224 rows
pca_scores <- scores(res_pcaNipals)

merged_pca <- merge(prots_anno, pca_scores, by = 0)
```
