...
ref_rat_trm <- rat_ref_proteome_2025_12_13 %>%
  select(Entry, Gene.Names, Protein.families, Keywords)
```
...
mapped_psum <- left_join(mapped_psum_v3.2.2_EExAge,
  ...
  left_join(ref_rat_trm, by = join_by("Protein" == "Entry"))
...
mapped_comp <- mapped_comp_v3.2.2_EExAge_t1 %>%
  ...
  left_join(ref_rat_trm, by = join_by("Protein" == "Entry")) %>%
  filter(!(Label %in% c("HxA_EE_YNG_vs_SH_OLD", "HxA_EE_OLD_vs_SH_YNG"))) # 6 plots
...
my_psum_facet_theme <- 
  theme(
    ...
    axis.text.x = element_text(size = 6, color ="grey10"),
    axis.title.y= element_text(size = 9, color ="grey10"),
    axis.title.x = element_blank(),
    
    panel.spacing = unit(0.2, "cm"),  # Removes space between panel 
    legend.title = element_text(color = "grey10", face = "italic",  size = 9),
    legend.margin = margin(-5,0,0,0), # space between plot & legend
    legend.key.size = unit(0.5, "cm"), # size of legend key icon,
    axis.ticks = element_line(linewidth = .1, colour = "grey15"),
    panel.border = element_rect(fill = NA,color = "grey10",linewidth = 0.3), #plot bord
...
```{r}
neurotrophins <- mapped_comp %>%
  filter(str_detect(Protein.families, regex("growth", ignore_case = TRUE)))

# 
growth_factors <- mapped_comp %>%
  filter(str_detect(Keywords, regex("Growth factor", ignore_case = TRUE)))

developmental_protein <- mapped_comp %>%
  filter(str_detect(Keywords, regex("Developmental protein", ignore_case = TRUE)))

transcription_factors <- mapped_comp %>%
  filter(str_detect(description, regex("Transcription factor", ignore_case = TRUE)))

calcium_channel <- mapped_comp %>%
  filter(str_detect(Keywords, regex("Calcium Channel", ignore_case = TRUE))) 



calcium_prots <- mapped_comp %>%
    filter(grepl("^Grin1|Grin2|Camk|Calm|Ryr|Cacng|Cacna|Prkc|Prka|Gnb|Gng|Gna|
Itpr|Slc8", gene)) %>%
  filter(adj.pvalue <= 0.2) # 7 proteins


Ras_prots <- mapped_comp %>%
    filter(grepl("^Mapk3|Ras|Nras", gene)) %>%
  filter(adj.pvalue <= 0.2) # 7 proteins
...
```{r}
# Neurotrophins, growthfactors T Systems & Signalling


neurotrophins_list <- c("Ntrk2", "Ntrk3", "Ntf3", "Bdnf", 'Sort1',
                        'Mmp3', 'Mmp7', 'Ngfr', 'Plat', 'Furin', 'Plg',
                        'Hdgf', 'Igf2r')


# growth_factors_list <- c("Igf1", "Igf2",
#                         'Fgfr2', 'Vgf')


# #Calcium list
# filter(grepl("^Gria|^Grin|^Grm|Calm|Ryr||Cacn", gene))
...
EE_syn_check <- mapped_psum %>%
  ...
  # filter(gene %in% neurotrophins_list) %>%
  # filter(str_detect(Keywords, regex("Growth factor", ignore_case = TRUE))) %>%
    # filter(gene %in%  calcium_prots) %>%

  filter(grepl("^Grin1|Grin2|Camk2|Camk4|Calm|Ryr2|Cacng|Prkar2b|Prkaca|Prkac|Gnb14|Slc8", gene)) %>%
  ...
  # filter( #gene %in% c("Camk2a") | #glutamate
  #        grepl("^Gria|^Grin|^Grm|Grik|Slc17a6|Slc17a7|Slc17a8|Cacng|Reln|^Htr", gene)) %>%
